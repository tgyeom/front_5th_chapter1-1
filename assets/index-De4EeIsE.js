...
`,w=()=>{const{get:t}=n,{username:e,email:l,bio:a}=t();return`
    <div id="root">
      <div class="bg-gray-100 min-h-screen flex justify-center">
        <div class="max-w-md w-full">
          ${p()}
...
`,c="/front_5th_chapter1-1",E=()=>{const t=window.location.pathname;return t.startsWith(c)?t.slice(c.length)||"/":t},b={"/":y,"/login":$,"/profile":w,"*":A},u=()=>{const{get:t}=n,e=t(),l=E();if(e.username&&l==="/login"){r("/");return}if(!e.username&&l==="/profile"){r("/login");return}const a=b[l]||b["*"],s=document.getElementById("root");s&&(s.innerHTML=a())},r=t=>{const e=c+(t.startsWith("/")?t:`/${t}`);history.pushState(null,null,e),u()},g=document.getElementById("root");g.addEventListener("submit",t=>{if(t.target.id==="login-form"){t.preventDefault();const l={username:document.getElementById("username").value,email:"",bio:""};n.set(l),r("/");return}if(t.target.id==="profile-form"){t.preventDefault();const e={username:document.getElementById("username").value,email:document.getElementById("email").value,bio:document.getElementById("bio").value};n.set(e)}});g.addEventListener("click",t=>{var l;if(t.target.id==="logout"){t.preventDefault(),n.clear(),r("/login");return}if(t.target.id==="login"){r("/login");return}const e=t.target.closest("a");e&&((l=e.getAttribute("href"))!=null&&l.startsWith("/"))&&(t.preventDefault(),r(e.getAttribute("href")))});window.addEventListener("popstate",u);window.addEventListener("DOMContentLoaded",u);
